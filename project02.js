...
function playRound(round, difficulty) {
    ...
    while (remainAttempts > 0) {
        const guess = parseInt(readline_sync_1.default.question(`Enter your guess (Attempts Left : ${attempts} attempts)`));
...
function selectDifficulty() {
    ...
    const index = readline_sync_1.default.keyInSelect(choices, "Choose a difficulty level: ", { cancel: false });
...
function main() {
    ...
    while (playAgain) {
        ...
        playAgain = readline_sync_1.default.keyInYN("Do you want to play another round? ");
        round++;
    }
    console.log("\n Game Summary : ");
    results.forEach((res) => {
